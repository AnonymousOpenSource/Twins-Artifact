...
class KMMatcher:
    ...
    def __init__(self, weights, zero_edge=True):
    ...
    def _solve(self, verbose = False):
        ...
        matches = []
        for x in range(self.n):
            if self.weights[x, self.xy[x]] < 1e-5 and (not self.zero_edge):
                continue
            if verbose:
    ...
    def solve(self, verbose=False):
        ...
        for x, y in matches:
            if self.is_transpose:
                weights.append(self.weights[y, x])
            else:
                weights.append(self.weights[x, y])
        return sum_, matches, self.is_transpose, weights
